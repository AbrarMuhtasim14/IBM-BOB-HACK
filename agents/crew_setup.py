...
from agents.skill_matcher import create_skill_matcher_agent, create_skill_matching_task, SkillMatcherService
from agents.shift_planner import create_shift_planner_agent, create_shift_planning_task, ShiftPlannerService
...
class SmartShiftCrew:
    ...
    def _initialize_llm(self) -> WatsonxLLM:
        ...
        if not self.settings.validate_watsonx_credentials():
            logger.warning("watsonx.ai credentials not configured. Using mock LLM.")
            # Return a mock LLM for development without credentials
            return None
    ...
    def process_overload_request(
        ...
        """
        Process an overload request and generate recommendations.
        ...
        try:
            ...
            # Step 1: Find candidates using skill matcher service
            ...
            if not candidates:
                return {
                    'success': False,
                    'error': f'No available workers found with {skill_requirement} skill'
                }
            
            # Step 2: Rank candidates
            ranked_candidates = self.skill_matcher_service.rank_candidates(candidates)
            
            # Step 3: Select best candidate using shift planner service
            ...
            return {
                ...
                'all_candidates': ranked_candidates[:3],  # Top 3 for reference
                'reasoning': reasoning
            }
    ...
    def _generate_reasoning(
        ...
        Args:
            ...
            all_candidates: All candidate workers
        ...
        reasoning = (
            f"Recommend moving {selected_worker['name']} from {from_zone} to {target_zone}. "
        )
        
        # Skill match reasoning
        if match_type == 'primary':
            reasoning += f"{skill.capitalize()} is their primary skill, ensuring high performance. "
        else:
            reasoning += f"They have {skill} as a transferable skill, making them well-suited for this role. "
        
        # Load reasoning
        if load_status == 'Low':
            reasoning += f"{from_zone} is currently at low load and can spare this worker. "
        elif load_status == 'Medium':
            reasoning += f"{from_zone} has moderate load but can accommodate this temporary shift. "
        
        # Comparison reasoning
        if len(all_candidates) > 1:
            reasoning += f"Selected over {len(all_candidates) - 1} other candidates due to optimal skill match and zone capacity."
...
def create_smartshift_crew(retriever, data_loader, embedder) -> SmartShiftCrew:
    """
